...
trainloader = DataLoader(traindataset, batch_size=BATCH_SIZE, shuffle=True, drop_last=True, num_workers=12)
validloader = DataLoader(validdataset, batch_size=BATCH_SIZE, shuffle=True, drop_last=True, num_workers=12)
...
df = pd.read_csv('predict_dataset.csv')
user_for_predict = df['user_id'].tolist()
...
for user in tqdm.tqdm(user_for_predict):
    # 将用户已经交互过的物品排除
